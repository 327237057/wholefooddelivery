...
It initializes a  webdriver, for which if you don't have one install it from: https://chromedriver.chromium.org/ for Chrome and https://github.com/mozilla/geckodriver/releases for FireFox. *Make sure the driver version matches your brower version!*

You'll have to update the path of the this installed webdriver under: ```python driver = webdriver.Chrome(executable_path="<PATH_TO_WEBDRIVER>")``` if its not the default location your OS needs. Similarly, for FireFox ```python driver = webdriver.Firefox(executable_path="<PATH_TO_WEBDRIVER>")```
...
1. Run the requirements.txt (```$ python3.8 -m pip install -r requirements.txt```)
2. Run amazon_fresh_delivery_slot_chrome.py (``` $ python3.8 amazon_fresh_delivery_slot_chrome.py```)
